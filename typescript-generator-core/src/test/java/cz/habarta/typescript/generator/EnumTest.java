...
public class EnumTest {
    ...
    public void testJavaLangEnum1() {
        final Settings settings = TestUtils.settings();
        final String output = new TypeScriptGenerator(settings)
                .generateTypeScript(Input.from(Child.NoEnumFactory.class));
    ...
    enum SideUsingToString {
        // No @JsonProperty set here because since 2.16 @JsonProperty has a higher precedence than toString()
        Left("left-side"),
        // No @JsonProperty set here because since 2.16 @JsonProperty has a higher precedence than toString()
        Right("right-side");
    ...
    public void testEnumMapKeys_MixedEnum() {
        ...
        settings.mapEnum = EnumMapping.asUnion;
        final String output = new TypeScriptGenerator(settings)
                .generateTypeScript(Input.from(ClassWithMixedEnum.class));
    ...
    public void testEnumMapKeys_NumberEnum() {
        ...
        settings.mapEnum = EnumMapping.asNumberBasedEnum;
        final String output = new TypeScriptGenerator(settings)
                .generateTypeScript(Input.from(ClassWithNumberEnum.class));
