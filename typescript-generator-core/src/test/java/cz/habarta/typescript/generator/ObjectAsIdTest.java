...
public class ObjectAsIdTest {
    ...
    public void testJacksonNestedMaps() throws JsonProcessingException {
        ...
        wrapper.listOfMapOfTestObjectA = Collections.singletonList(generateMap(testObjectA, testObjectA));
        wrapper.listOfMapOfTestObjectB = Collections.singletonList(generateMap(testObjectB, testObjectB));
        wrapper.listOfMapOfTestObjectC = Collections.singletonList(generateMap(testObjectC, testObjectC));
        wrapper.listOfMapOfTestObjectD = Collections.singletonList(generateMap(testObjectD, testObjectD));
        wrapper.listOfMapOfTestObjectE = Collections.singletonList(generateMap(testObjectE, testObjectE));
        final ObjectMapper objectMapper = Utils.getObjectMapper();
        objectMapper.disable(SerializationFeature.INDENT_OUTPUT);
        final String json = objectMapper.writeValueAsString(wrapper);
