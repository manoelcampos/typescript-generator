...
public class Jackson3ParserTest {
    ...
    @Test
    public void test() {
        final Jackson3Parser jacksonParser = getJackson3Parser();
    ...
    public void testChangedNameProperty() {
        final Jackson3Parser jacksonParser = getJackson3Parser();
        final Model model = jacksonParser.parseModel(DummyBeanJackson3.class);
        Assertions.assertTrue(model.getBeans().size() > 0);
        final BeanModel beanModel = model.getBeans().get(0);
        Assertions.assertEquals("DummyBeanJackson3", beanModel.getOrigin().getSimpleName());
        Assertions.assertTrue(beanModel.getProperties().size() > 0);
    ...
    public void testTaggedUnion() {
        ...
        Assertions.assertEquals("Jackson3ParserTest$SubTypeDiscriminatedByName3", bean3.getDiscriminantLiteral());
        Assertions.assertEquals("Jackson3ParserTest$SubTypeDiscriminatedByName4", bean4.getDiscriminantLiteral());
    }
    ...
    static Jackson3Parser getJackson3Parser() {
        final Settings settings = new Settings();
        settings.jackson3Modules.add(NamedSubtypeModule.class);

        return new Jackson3Parser(settings, new DefaultTypeProcessor());
    }

    public static class DummyBeanJackson3 {
    ...
    public static class IdSerializer extends ValueSerializer<Identifyable> {
        @Override
        public void serialize(Identifyable value, JsonGenerator gen, SerializationContext ctxt)
    ...
    @Test
    public void testJacksonIdSerializer() throws JacksonException {
    ...
    @Test
    public void testJacksonLocalDateTimeDeserializer() throws JacksonException {
    ...
    public void testSerializerAndDeserializer() {
        final Settings settings = TestUtils.settings(true);
        settings.jackson3Configuration = new Jackson3ConfigurationResolved();
        settings.jackson3Configuration.serializerTypeMappings = Collections.singletonMap(IdSerializer.class,
                "{ id: string }");
        settings.jackson3Configuration.deserializerTypeMappings = Collections
